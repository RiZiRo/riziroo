...
- **Dynamic Island** — a notch-style bar centrepiece that expands into media, a clock, weather, search and OSD surfaces.
...
- **A native typing test** — a Monkeytype-style test in pure QML: word / quote / zen modes, punctuation and numbers, difficulty levels, an on-screen keyboard, result graphs, saved history with replay, and a theme editor.
...
**How do I see my keybinds?** Open the launcher (`SUPER`) and type `<` — it lists every configured keybind.

**Why doesn't Settings have a search bar?** The launcher already does that job. Open it (`SUPER`) and type what you want (`wallpaper`, `bar`, `blur`); it matches page names and jumps you there.
